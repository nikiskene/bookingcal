...
export const handler: Handler = async (event) => {
  ...
  try{
    ...
    if(body.website) return {statusCode:400,body:JSON.stringify({error:'Invalid submission'})};
    ...
    if(!name||!email||!startUtc||!Number.isFinite(duration)) return {statusCode:400,body:JSON.stringify({error:'Please complete the required fields'})};
    ...
    if(!template) return {statusCode:404,body:JSON.stringify({error:'Booking link not found'})};
    if(!template.durations.includes(duration)) return {statusCode:400,body:JSON.stringify({error:'Invalid duration'})};
    if(!template.purposes.includes(purpose)) return {statusCode:400,body:JSON.stringify({error:'Please choose a valid purpose'})};
    if(!template.meetingMethods.includes(meetingMethod as 'zoom'|'teams')) return {statusCode:400,body:JSON.stringify({error:'Please choose a meeting method'})};
    for(const q of template.questions){const a=answers.find((x)=>x.questionId===q.id)?.answer?.trim()||'';if(q.required&&!a)return {statusCode:400,body:JSON.stringify({error:`Please answer: ${q.label}`})};}
    if(template.freeTextEnabled&&template.freeTextRequired&&!message) return {statusCode:400,body:JSON.stringify({error:`Please complete: ${template.freeTextLabel}`})};
    ...
    if(!selected) return {statusCode:409,body:JSON.stringify({error:'That time is no longer available. Please choose another slot.'})};
  ...
  }catch(e){return {statusCode:500,body:JSON.stringify({error:e instanceof Error?e.message:'Booking failed'})};}
};
