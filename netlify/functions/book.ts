...
import { encodeBookingMeta, parseBookingMeta, type BookingMeta } from './_shared/manage';
...
function siteUrl(event:any){
  const candidate=process.env.URL||event.headers?.origin||'https://nikiskenecal.netlify.app';
  const url=new URL(candidate);
  if(url.protocol!=='https:'&&url.protocol!=='http:')throw new Error('BookingCal site URL is invalid');
  return url.origin;
}
...
export const handler: Handler = async (event) => {
  ...
  try{
    ...
    const meta:BookingMeta={customerName:name,customerEmail:email,viewerTimezone,meetingMethod:meetingMethod as 'zoom'|'teams',purpose,templateSlug:slug,manageToken,manageUrl,duration};
    ...
    const persisted=parseBookingMeta(created?.body?.content||'');
    if(persisted?.manageToken!==manageToken||persisted.manageUrl!==manageUrl)throw new Error('Microsoft calendar event did not preserve booking management metadata');
